...
def test_all(testdata_loader, model):
    ...
    losses_all = []
    with torch.no_grad():
        for i, batch_data in enumerate(testdata_loader):
            # 处理不同数量的返回值
            if len(batch_data) == 3:
                batch_xs, batch_ys, batch_toas = batch_data
            elif len(batch_data) >= 5:
                # 即使vis=False，数据加载器可能仍然返回5个值
                batch_xs, batch_ys, batch_toas = batch_data[0], batch_data[1], batch_data[2]
            else:
                batch_xs, batch_ys, batch_toas = batch_data[0], batch_data[1], batch_data[2]
...
def train_eval():
    ...
    # building model
    model = DSTA(train_data.dim_feature, p.hidden_dim, p.latent_dim,
...
def test_eval():
    ...
    if p.visualize:
        ...
        vis_dir = result_dir  # 确保vis_dir不为None
    ...
    if p.dataset == 'dad':
        from src.DataLoader import DADDataset
        test_data = DADDataset(data_path, p.feature_name, 'testing', toTensor=True, device=device, vis=False)  # 改为False

    elif p.dataset == 'crash':
        from src.DataLoader import CrashDataset
        test_data = CrashDataset(data_path, p.feature_name, 'test', toTensor=True, device=device, vis=False)  # 改为False
    else:
        raise NotImplementedError
    testdata_loader = DataLoader(dataset=test_data, batch_size=p.batch_size, shuffle=False, drop_last=True)
    ...
    # building model
    model = DSTA(test_data.dim_feature, p.hidden_dim, p.latent_dim,
    ...
    if p.evaluate_all:
        ...
        if not os.path.exists(result_file):
            ...
            # 确保加载的模型在正确设备上
            model = model.to(device)
            # 使用 test_all 而不是 test_all_vis，禁用可视化
            all_pred, all_labels, all_toas, losses_all = test_all(testdata_loader, model)
            # 保存预测结果（不包含vis_data）
            ...
            all_pred, all_labels, all_toas = \
                all_results['pred'], all_results['label'], all_results['toas']
        ...
        print("Test Results:")
        print(f"AP: {AP:.4f}")
        print(f"mTTA: {mTTA:.2f}")
        print(f"TTA_R80: {TTA_R80:.2f}")
        print(f"P_R80: {P_R80:.4f}")
        print(f"AP_video: {AP_video:.4f}")
